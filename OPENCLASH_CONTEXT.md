...
高倍率组仅为 `🇺🇸 高倍率美国`、`🇸🇬 高倍率新加坡`、`🇭🇰 高倍率香港`、`🇹🇼 高倍率台湾`、`🇯🇵 高倍率日本`。高倍率节点严格匹配数值大于 `1x` 的倍率标签，例如 `1.01x`、`1.5x`、`2x`、`15x`；`0.5x`、`1x`、`1.0x`、`1.00x` 不属于高倍率，仍归普通组。五个特权国家的普通组排除高倍率节点，并继续排除精确的 `0.1x` 节点；高倍率与普通组的倍率筛选不重叠。所有策略选择中的五国节点均按“普通 → 同国高倍率”相邻排列。`进阶IEPL`、`专线`、`原生`、`家宽`、`星链`、`CN2`、`IPLC`、普通 `IEPL`、`进阶`不再作为高倍率条件。
