...
7. 不做 `0.1x` 的全局订阅排除；仅香港、台湾、美国、日本、新加坡五个普通国家组应排除精确的 `0.1x` 节点。高倍率组和其他国家不受此条件影响。
...
所有自动节点组均为 `fallback`。香港、台湾、美国、日本、新加坡五国的普通组和对应高倍率组均按订阅顺序保留第一个健康节点，仅在失联时向后切换；`♻️ 自动选择` 也使用同一机制。

高倍率组仅为 `🇺🇸 US High`、`🇸🇬 SG High`、`🇭🇰 HK High`、`🇹🇼 TW High`、`🇯🇵 JP High`。高倍率节点严格匹配数值大于 `1x` 的倍率标签，例如 `1.01x`、`1.5x`、`2x`、`15x`；`0.5x`、`1x`、`1.0x`、`1.00x` 不属于高倍率，仍归普通组。五个特权国家的普通组排除高倍率节点，并继续排除精确的 `0.1x` 节点；高倍率与普通组的倍率筛选不重叠。所有策略选择中的五国节点均按“普通 → 同国高倍率”相邻排列。`进阶IEPL`、`专线`、`原生`、`家宽`、`星链`、`CN2`、`IPLC`、普通 `IEPL`、`进阶`不再作为高倍率条件。

韩国及其他独立展示国家均为 `fallback`，按国家保留当前健康节点、失联才切换，不受五个特权国家的高倍率/普通互斥筛选影响。`🧊 冷门节点`归集澳门、泰国、印度尼西亚、越南、菲律宾、新西兰、巴西、墨西哥、智利、荷兰、挪威、阿联酋、沙特阿拉伯、尼日利亚，也采用 `fallback` 并保留全部节点。
