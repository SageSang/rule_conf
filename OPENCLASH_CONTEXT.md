...
该组的选择顺序以 `🇯🇵 JP`、`🇯🇵 JP High` 开头，随后保留其他地区、冷门、手动和自动出口。Pixiv 规则当前包含 `booth.pm`、`fanbox.cc`、`pixiv.cat`、`pixiv.co.jp`、`pixiv.me`、`pixiv.net`、`pixiv.org`、`pximg.net`；DLsite 规则当前包含 `chobit.cc`、`ci-en.jp`、`ci-en.net`、`dlaf.jp`、`dlsite.com`、`dlsite.jp`、`dlsitestudio.com`、`nijiyome.jp`、`triokini.com`、`dlsite.com.tw`；另外以内联规则补充 `fanza.cc`、`fantia.jp`。
